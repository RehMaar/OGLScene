...
#define GEN_OBJECT(_name, _txr_path, _obj_path, _txr_ctr, _x, _y, _z, _tr) \
    ...
    rock_trans.scale(25, 25, 25);

    GEN_OBJECT(
        cube,
        "textures/blue.jpg", //"models/cube/cube.png",
        "models/cube/test_model.obj",
        txr_ctr++,
        -67, 0, -5,
        0.5
    );
    cube_trans.scale(10, 10, 10);

    GEN_OBJECT(
        cube2,
        "textures/gray.jpg", //"models/cube/cube.png",
        "models/cube/test_model.obj",
        txr_ctr++,
        -107, 0, -5,
        1.0
    );
    cube2_trans.scale(10, 10, 10);
    ...
    vec3 const_light_pos1 = vec3(-10, 49.967182, 0);
    vec3 light_tar1 = vec3(-0.767405, -0.609159, 0.200037);
    vec3 light_up1  = vec3(-0.589462, 0.793048, 0.153653);

    vec3 const_light_pos2 = vec3(-100, 49.967182, -70);
    vec3 light_tar2 = vec3(0, -0.609159, 1);
    vec3 light_up2  = vec3(-0.589462, 0.793048, 0.153653);

    vec3 light_pos3 = vec3(95.404121, 75.332268, 3.375690);
    vec3 light_tar3 = vec3(-0.364467, -0.572868, -0.734157);
    vec3 light_up3 = vec3(-0.254733, 0.819648, -0.513116);
    ...
    spot_light sl1;
    sl1.pl.color = vec3(1, 0, 1);

    spot_light sl2;
    sl2.pl.color = vec3(0, 1, 1);
    ...
    shadow.add_object(&cube2, &cube2_trans);
    shadow.add_object(&sphere, &sphere_trans);
    ...
    t_map.add_object(&cube, &cube_trans, cube_transparency);
    t_map.add_object(&cube2, &cube2_trans, cube2_transparency);
//    t_map.add_object(&tower, &tower_trans, tower_transparency);
    t_map.add_object(&sphere2, &sphere2_trans, sphere2_transparency);
    t_map.add_object(&sphere, &sphere_trans, sphere_transparency);
...
#define RENDER(_obj)                                            \
   do {                                                         \
        ...
        light.set_material_reflect_i(40);
        light.set_specular_power(40);
        RENDER(rock);
        light.set_material_reflect_i(10);
        light.set_specular_power(10);
